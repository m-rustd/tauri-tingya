...
#[get("/mv_list")]
fn mv_list() -> Json<Vec<MVItem>> {
    let mut list: Vec<MVItem> = Vec::new();
    for _ in 1..11 {  
      list.push(MVItem { 
        cover: String::from("https://img1.kuwo.cn/wmvpic/324/78/3/4093527818.jpg"), 
        title: String::from("The Lighthouse Keeper"), 
        author: String::from("Sam Smith"),
      });
    } 
    Json(list)
}
...
#[launch]
fn rocket() -> _ {
    rocket::build().mount("/", routes![list, get_categories, mv_list]) //挂载
}
